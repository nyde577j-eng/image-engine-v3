...
function DynamicBarsIcon({ level }: { level: string }) {
  ...
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      <rect
        x="1.5" y="8" width="2.5" height="4.5" rx="1"
        fill="currentColor"
        className="transition-opacity duration-300"
        opacity={1}
      />
      <rect
        x="5.75" y="5" width="2.5" height="7.5" rx="1"
        fill="currentColor"
        className="transition-opacity duration-300"
        opacity={isMediumOrHigh ? 1 : 0.3}
      />
      <rect
        x="10" y="2" width="2.5" height="10.5" rx="1"
        fill="currentColor"
        className="transition-opacity duration-300"
    ...
    >
      ...
      >
        <span
          ...
          className={cn(
            'm-1 flex size-4 items-center justify-center rounded-full bg-background/90 text-foreground/70 shadow-sm',
            'transition-all duration-200 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] hover:bg-background hover:text-foreground hover:scale-110',
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    useEffect(() => {
      ...
      el.style.transition = '';
      const newH = Math.max(68, Math.min(scrollH, 160));
      el.style.height = `${newH}px`;
      setTextareaHeight(newH);
      setIsScrolling(scrollH > 160);
          ...
          >
            <style dangerouslySetInnerHTML={{
              __html: `
                ...
                .prompt-scrollbar:hover::-webkit-scrollbar-thumb { background: rgba(150,150,150,0.3); }
              `,
            }} />
                ...
                <div
                  ...
                  onMouseLeave={() =>
                    setHoverStyle((prev) => ({
                      ...prev,
                      opacity: 0,
                      transform: (prev.transform as string)?.replace('scale(1)', 'scale(0.95)'),
