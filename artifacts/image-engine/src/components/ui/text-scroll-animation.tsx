...
type CharacterProps = {
  ...
  index: number;
  centerIndex: number;
  scrollYProgress: ReturnType<typeof useTransform> | ReturnType<typeof useScroll>["scrollYProgress"];
};

const CharacterV1 = ({ char, index, centerIndex, scrollYProgress }: CharacterProps) => {
  const isSpace = char === " ";
  const distanceFromCenter = index - centerIndex;
  const x = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [distanceFromCenter * 50, 0]);
  const rotateX = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [distanceFromCenter * 50, 0]);
  return (
    <motion.span
...
const CharacterV2 = ({ char, index, centerIndex, scrollYProgress }: CharacterProps) => {
  const distanceFromCenter = index - centerIndex;
  const x = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [distanceFromCenter * 50, 0]);
  const scale = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [0.75, 1]);
  const y = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [Math.abs(distanceFromCenter) * 50, 0]);
  return (
    <motion.img
      src={char}
      alt=""
...
const CharacterV3 = ({ char, index, centerIndex, scrollYProgress }: CharacterProps) => {
  const distanceFromCenter = index - centerIndex;
  const x = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [distanceFromCenter * 90, 0]);
  const rotate = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [distanceFromCenter * 50, 0]);
  const y = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [-Math.abs(distanceFromCenter) * 20, 0]);
  const scale = useTransform(scrollYProgress as ReturnType<typeof useScroll>["scrollYProgress"], [0, 0.5], [0.75, 1]);
  return (
    <motion.img
      src={char}
  ...
  return (
    <ReactLenis root>
      <main className="w-full bg-white">
        ...
        {/* Section 2 — icons fly in from sides */}
        <div
          ref={targetRef2}
