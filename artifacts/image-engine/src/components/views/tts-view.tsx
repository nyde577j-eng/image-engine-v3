...
function AudioCard({ url, name, voice, duration, onDelete }: {
  ...
  const fmt = (s: number) => `${Math.floor(s/60)}:${String(Math.floor(s%60)).padStart(2,'00')}`;
...
function VoicePreview({ audioUrl, onStop }: { audioUrl: string; onStop: () => void }) {
  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{ marginTop:8, padding:'8px 10px', background:'rgba(255,77,31,.08)', border:'1px solid rgba(255,77,31,.2)', borderRadius:10, display:'flex', alignItems:'center', gap:8 }}>
      ...
      <button onClick={toggle} style={{ width:28, height:28, borderRadius:'50%', background:'var(--acc)', border:0, color:'#fff', display:'grid', placeItems:'center', flexShrink:0, cursor:'pointer' }}>
        {playing
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', gap:6 }}>
        <div style={{ flex:1, height:3, background:'rgba(255,77,31,.2)', borderRadius:99, overflow:'hidden', cursor:'pointer' }}
          onClick={e => { e.stopPropagation(); if (!audioRef.current || !dur) return; const rect = e.currentTarget.getBoundingClientRect(); audioRef.current.currentTime = ((e.clientX - rect.left) / rect.width) * dur; }}>
          <div style={{ height:'100%', width:`${dur ? (progress/dur)*100 : 0}%`, background:'var(--acc)', borderRadius:99, transition:'width .1s' }} />
        </div>
        <span style={{ fontFamily:'var(--mono)', fontSize:9.5, color:'var(--acc)', flexShrink:0 }}>{fmt(progress)}/{fmt(dur || 0)}</span>
      </div>

      <button onClick={e => { e.stopPropagation(); audioRef.current?.pause(); onStop(); }}
        style={{ background:'none', border:0, cursor:'pointer', color:'var(--mut)', padding:2, display:'grid', placeItems:'center' }} title="Close preview">
...
function VoiceCard({ voice, selected, onSelect, playingPreviewId, onPreviewToggle }: {
  ...
  const handlePreviewClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onPreviewToggle(isPreviewing ? null : voice._id);
  };

  return (
    <div style={{ border: selected ? '1px solid var(--acc)' : '1px solid var(--line2)', borderRadius:14, padding:12, background: selected ? 'var(--accsoft)' : 'var(--card)', boxShadow: selected ? '0 0 0 3px rgba(255,77,31,.14)' : 'none', transition:'.15s', width:'100%', display:'flex', flexDirection:'column', gap:0 }}>
      <button onClick={onSelect} style={{ display:'flex', gap:10, alignItems:'center', background:'none', border:0, cursor:'pointer', textAlign:'left', width:'100%', padding:0 }}>
        ...
        <div style={{ minWidth:0, flex:1 }}>
          ...
          <span style={{ fontFamily:'var(--mono)', fontSize:10.5, color:'var(--mut)' }}>{voice.languages?.slice(0,2).join(' · ') ?? '—'}</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          {sampleUrl && (
            <button onClick={handlePreviewClick} title={isPreviewing ? 'Stop preview' : 'Preview voice sample'}
              style={{ width:26, height:26, borderRadius:'50%', background: isPreviewing ? 'var(--acc)' : 'rgba(255,77,31,.15)', border:'1px solid rgba(255,77,31,.3)', color: isPreviewing ? '#fff' : 'var(--acc)', display:'grid', placeItems:'center', cursor:'pointer', transition:'.15s', flexShrink:0 }}>
              {isPreviewing
          ...
          {selected && <div style={{ width:8, height:8, borderRadius:'50%', background:'var(--acc)' }} />}
        </div>
      </button>
      {isPreviewing && sampleUrl && <VoicePreview audioUrl={sampleUrl} onStop={() => onPreviewToggle(null)} />}
    </div>
  );
}
    ...
    <div style={{ padding:'clamp(16px,3vw,30px)', paddingBottom:80, maxWidth:1460, margin:'0 auto' }}>
      ...
      <div className="vhead">
        <div>
          ...
          <p>Convert text to natural voice, or browse the voice library.</p>
        </div>
      </div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                {FORMATS.map(f => (
                  <button key={f} className={`chip${format===f?' on':''}`} onClick={() => setFormat(f)}
                    style={{ flex:1, textAlign:'center', textTransform:'uppercase' }}>{f}</button>
                ))}
              </div>
            </div>
